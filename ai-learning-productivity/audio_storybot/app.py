...
app = FastAPI(title="audio_storybot", version="1.2.0")
...
@app.get("/")
def health():
    return {"ok": True, "model": OLLAMA_MODEL, "ollama_url": OLLAMA_URL, "output_dir": str(OUTPUT_DIR)}

@app.post("/explain")
def explain(req: ExplainRequest):
    # 1) Load source
    code, name = load_source(file_url=req.file_url, file_path=req.file_path)

    # 2) Build instructed prompt
    prompt = build_prompt(source_code=code, controls=req.controls, mode=req.mode)

    # 3) LLM -> story/script
    story = call_ollama_generate(model=OLLAMA_MODEL, prompt=prompt)

    # 4) TTS -> mp3 (in-memory)
    audio = tts_gtts(story, lang=req.language)

    # 5) Optional BGM mix
    if req.bg_music_path:
        audio = mix_bgm(audio, req.bg_music_path)

    # 6) Server-side save to ./output
    safe_name = pathlib.Path(name).stem or "file"
    ts = time.strftime("%Y%m%d_%H%M%S")
    out_name = f"{safe_name}_{ts}.mp3"
    out_path = OUTPUT_DIR / out_name
    out_path.write_bytes(audio)

    return {
        "meta": {
            "title": f"Explainer for {name}",
            "logline": "Short, precise, second-person walkthrough.",
